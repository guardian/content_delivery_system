...
class VSACL
    ...
    def to_xml
        builder=Nokogiri::XML::Builder.new do |xml|
            xml.AccessControlListDocument('xmlns'=>"http://xml.vidispine.com/schema/vidispine") {
...
class VSAccess
    ...
    def initialize(user: nil,group: nil,permission: nil,recursive: false,xmldoc: nil)
        if(xmldoc)
            ...
            if(recursive_text.downcase == "true")
                @recursive=true
            else
                @recursive=false
            end
    ...
    def to_xml(builder: nil)
        if(builder)
            ...
            builder=Nokogiri::XML::Builder.new do |xml|
                xml.AccessControlDocument('xmlns'=>"http://xml.vidispine.com/schema/vidispine") {
                    xml.recursive(@recursive)
                    xml.permission(@permission)
                    xml.user(@user) if(@user)
                    xml.group(@group) if(@group)
