...
class CDSResponder
  ...
  def initialize(arn, routename, arg, notification, logging_conf)
    @routename=routename
    @cdsarg=arg
  ...
  begin
    ...
    OptionParser.new do |opts|
      ...
      opts.on("-c", "--config CONFIGFILE", "Path to the configuration file.  This should contain the following:",
              ...
              "\tlogging-host={database host}",
              "\tlogging-db={database-name}",
              "\tlogging-user={database-user}",
              "\tlogging-passwd={database-password}") do |cfg|
