...
class MetadataGroup
  ...
  def each
    @node.xpath('vs:field', 'vs' => 'http://xml.vidispine.com/schema/vidispine').each do |fieldnode|
      name = fieldnode.xpath('vs:name', 'vs' => 'http://xml.vidispine.com/schema/vidispine').text
      values = []
      fieldnode.xpath('vs:value', 'vs' => 'http://xml.vidispine.com/schema/vidispine').each do |valnode|
        values << _transformer(valnode.text)
      end
