...
class SearchParser < Nokogiri::XML::SAX::Document
  ...
  def start_document
    puts 'SearchParser: startDocument' if @debug
    @current_tree = []
    @root_name = nil
    @item_name = nil
  ...
  def end_document
    puts 'SearchParser: endDocument' if @debug
  end

  def start_element(name, attrs = [])
    puts "SearchParser: startElement: #{name} #{attrs}" if @debug

    @current_tree << name
    ap @current_tree if @debug
  ...
  def end_element(name)
    puts "SearchParser: endElement: #{name}" if @debug
  ...
  def characters(string)
    puts 'SearchParser: characters' if @debug

    current_tag = @current_tree[-1]
    parent_tag = @current_tree[-2]
    puts "\tparent tag: #{current_tag}" if @debug

    case current_tag
    when 'id'
      if (parent_tag == 'collection') ||
         (parent_tag == 'item') ||
         (parent_tag == 'content')
  ...
  def cdata_block(string)
    puts "SearchParser: cdata_block: #{string}" if @debug
  end
...
class VSSearchCriterion
  ...
  def output_xml(xmlbuilder)
    if @invert
      xmlbuilder.operator('operation' => 'NOT') do
...
class VSSearch < VSApi
  ...
  def results(start: nil, number: nil, withinCollection: nil)
    builder = Nokogiri::XML::Builder.new do |xml|
      xml.ItemSearchDocument('xmlns' => 'http://xml.vidispine.com/schema/vidispine') do
    ...
    puts "Got a total of #{saxclass.hits} hits" if @debug
  end # def results

  def call_make_xml(xmlbuilder, mdhash)
    output_xml_fieldgroup(xmlbuilder, mdhash, top: false)
  end
end # class VSSearch
